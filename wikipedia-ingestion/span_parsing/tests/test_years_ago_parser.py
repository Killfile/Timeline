...
class TestYearsAgoParser:
    ...
    def test_250000_years_ago(self):
        ...
        span = self.parser.parse("250,000 years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 250000
    ...
    def test_170000_years_ago(self):
        ...
        span = self.parser.parse("170,000 years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 170000
        assert span.start_year == abs(expected_year)
        assert span.start_year_is_bc is True
    
    def test_10000_years_ago_no_comma(self):
        """Test '10000 years ago' (no comma) parses correctly."""
        span = self.parser.parse("10000 years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 10000
        assert span.start_year == abs(expected_year)
    
    # Million multiplier
    def test_2_million_years_ago(self):
        """Test '2 million years ago' parses correctly."""
        span = self.parser.parse("2 million years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 2000000
        assert span.start_year == abs(expected_year)
        assert span.start_year_is_bc is True
    
    def test_2_5_million_years_ago(self):
        """Test '2.5 million years ago' parses correctly."""
        span = self.parser.parse("2.5 million years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 2500000
        assert span.start_year == abs(expected_year)
    
    # Thousand multiplier
    def test_50_thousand_years_ago(self):
        """Test '50 thousand years ago' parses correctly."""
        span = self.parser.parse("50 thousand years ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 50000
        assert span.start_year == abs(expected_year)
    
    # Range patterns
    def test_5_2_million_years_ago_range(self):
        """Test '5-2 million years ago' range parses correctly."""
        span = self.parser.parse("5-2 million years ago", 2000, False)
        assert span is not None
        expected_start = self.current_year - 5000000
        expected_end = self.current_year - 2000000
    ...
    def test_300000_100000_years_ago_range(self):
        ...
        span = self.parser.parse("300,000-100,000 years ago", 2000, False)
        assert span is not None
        expected_start = self.current_year - 300000
        expected_end = self.current_year - 100000
        assert span.start_year == abs(expected_start)
        assert span.end_year == abs(expected_end)
    ...
    def test_singular_year(self):
        ...
        span = self.parser.parse("1000 year ago", 2000, False)
        assert span is not None
        expected_year = self.current_year - 1000
        assert span.start_year == abs(expected_year)
    ...
    def test_years_ago_not_at_start(self):
        ...
        span = self.parser.parse("About 250,000 years ago", 2000, False)
        assert span is None
