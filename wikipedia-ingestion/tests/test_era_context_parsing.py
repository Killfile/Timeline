...
def test_parse_span_inherits_bc_from_page_context_for_year_only():
    ...
    span = YearsParseOrchestrator.parse_span_from_bullet("970 — Something happened", 970, assume_is_bc=True)
...
def test_parse_span_defaults_ad_when_no_context_for_year_only():
    span = YearsParseOrchestrator.parse_span_from_bullet("970 — Something happened", 970, assume_is_bc=None)
    assert span is not None
    assert span.is_bc is False


def test_parse_span_explicit_era_overrides_page_context():
    span = YearsParseOrchestrator.parse_span_from_bullet("970 BC — Something happened", 970, assume_is_bc=False)
    assert span is not None
    assert span.start_year == 970
    assert span.is_bc is True
...
def test_parse_range_inherits_bc_when_no_era_markers_present():
    span = YearsParseOrchestrator.parse_span_from_bullet("970–968 — Something happened", 970, assume_is_bc=True)
    assert span is not None
    assert span.start_year == 970
